...
/**
 * Helper: Extracts url and fileType from a document value.
 * Supports both JSON objects { url, fileType } and legacy plain string URLs.
 */
const resolveDocValue = (docValue) => {
    if (!docValue) return { url: null, fileType: null };

    // JSON object from new schema
    if (typeof docValue === 'object' && docValue.url) {
        return { url: docValue.url, fileType: docValue.fileType || 'image' };
    }

    // Legacy plain string URL
    if (typeof docValue === 'string') {
        const isPdf = docValue.includes('/raw/upload') || docValue.toLowerCase().endsWith('.pdf');
        return { url: docValue, fileType: isPdf ? 'pdf' : 'image' };
    }
...
const DocumentPreviewModal = ({ isOpen, onClose, documentUrl, documentLabel }) => {
    ...
    const [resolvedType, setResolvedType] = useState('image');

    useEffect(() => {
        if (!isOpen || !documentUrl) return;

        const { url, fileType } = resolveDocValue(documentUrl);
        if (!url) return;

        let finalUrl = url;
        if (!finalUrl.startsWith('http')) {
            finalUrl = `${window.location.protocol}//${window.location.hostname}:5000/${finalUrl.replace(/\\/g, '/')}`;
        }
        setResolvedUrl(finalUrl);
        setResolvedType(fileType || 'image');
    }, [documentUrl, isOpen]);
            ...
            <div className="relative bg-white rounded-2xl shadow-2xl max-w-5xl w-full max-h-[90vh] flex flex-col overflow-hidden animate-fade-in">
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-slate-50/80">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <a href={resolvedUrl} target="_blank" rel="noopener noreferrer" className="h-9 px-3 rounded-lg bg-white border border-slate-200 text-slate-700 font-medium text-sm flex items-center gap-2 hover:bg-slate-50">
                            <ExternalLink size={14} className="text-slate-400" /> Open
                        </a>
                        <a href={resolvedUrl} download className="w-9 h-9 rounded-lg bg-white border border-slate-200 text-slate-700 flex items-center justify-center hover:bg-slate-50">
                            <Download size={16} />
                        </a>
                    ...
                    {isPDF ? (
                        <div className="w-full h-full flex flex-col items-center gap-4">
                            ...
                            <div className="bg-white/80 backdrop-blur border border-slate-200 px-4 py-2 flex justify-between items-center w-full rounded-lg shadow-sm">
                                ...
                                <a href={resolvedUrl} target="_blank" rel="noopener noreferrer" className="text-sm font-bold text-primary-600 hover:text-primary-700 flex items-center gap-1 bg-primary-50 px-3 py-1.5 rounded-md">
